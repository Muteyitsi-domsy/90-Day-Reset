...
const CrisisModal: React.FC<CrisisModalProps> = ({ severity, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in-fast" aria-modal="true" role="alertdialog">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg max-w-md w-full p-6 text-center border-2 border-red-500 dark:border-red-400">
        <h2 className="text-2xl font-light text-red-800 dark:text-red-300 mb-4">{content.title}</h2>
        <p className="text-md text-gray-700 dark:text-gray-300 leading-relaxed mb-4">{content.intro}</p>
        ...
        <button 
          onClick={onClose} 
          className="w-full mt-6 py-2 rounded-lg bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 font-medium hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors"
